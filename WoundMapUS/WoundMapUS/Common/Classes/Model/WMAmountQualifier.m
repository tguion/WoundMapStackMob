...
+ (void)seedDatabase:(NSManagedObjectContext *)managedObjectContext completionHandler:(WMProcessCallbackWithCallback)completionHandler
{
    ...
    @autoreleasepool {
        ...
        if (completionHandler) {
            completionHandler(nil, objectIDs, [WMAmountQualifier entityName], nil);
        }
    }
}
...
+ (NSSet *)relationshipNamesNotToSerialize
{
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        PropertyNamesNotToSerialize = [NSSet setWithArray:@[WMAmountQualifierRelationships.measurementValues]];
    });
    return PropertyNamesNotToSerialize;
}
...
{
    if ([[WMAmountQualifier attributeNamesNotToSerialize] containsObject:propertyName] || [[WMAmountQualifier relationshipNamesNotToSerialize] containsObject:propertyName]) {
        return NO;
    }
    // else
    return YES;

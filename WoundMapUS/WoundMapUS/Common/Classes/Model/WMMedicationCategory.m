...
+ (NSArray *)relationshipNamesNotToSerialize
{
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        PropertyNamesNotToSerialize = @[WMMedicationCategoryRelationships.medications];
    });
    return PropertyNamesNotToSerialize;
}
...
{
    if ([[WMMedicationCategory attributeNamesNotToSerialize] containsObject:propertyName]) {
        return NO;
    }
    // else
    return YES;

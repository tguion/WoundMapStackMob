...
- (void)drawRect:(CGRect)rect
{
    ...
    CGSize textSize = [dayText sizeWithAttributes:textAttributes];
    CGFloat textX, textY;
    textX = roundf(0.5f * (kTileSize.width - textSize.width));
    textY = 6.f + roundf(0.5f * (kTileSize.height - textSize.height));
    if (shadowColor) {
        NSDictionary *textShadowAttributes = @{NSFontAttributeName:font,
                                               NSForegroundColorAttributeName:shadowColor};
        [dayText drawAtPoint:CGPointMake(textX, textY) withAttributes:textShadowAttributes];
        textY += 1.f;
    }
    [dayText drawAtPoint:CGPointMake(textX, textY) withAttributes:textAttributes];
